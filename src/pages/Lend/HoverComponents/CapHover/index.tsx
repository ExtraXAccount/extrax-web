...
export default function CapHover(props: {
  ...
  const content = (
    <section className={cx('cap-hover', 'hover-content', `cap-hover-${type}`)}>
      <div className='cap-hover-title flex ai-ct'>
        <p>{type} cap: {addComma(max)}</p>
      ...
      <div className='cap-hover-content'>
        ...
        <section className='cap-hover-content-text flex ai-ct'>
          <p className='flex ai-ct gap-4'><i className='cap-hover-circle cap-hover-circle-main'></i>{type} used: {addComma(current)}</p>
          <p className='flex ai-ct gap-4'><i className='cap-hover-circle'></i>remaining: {addComma(max - current)} ({Math.trunc(100 - percent)}%)</p>
        </section>
      </div>
    </section>
  )
