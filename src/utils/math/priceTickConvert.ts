...
export function token1price2tick(
  price: number,
  token0decimals = 18,
  token1decimals = 18,
) {
...
export function token0price2tick(
  price: number,
  token0decimals = 18,
  token1decimals = 18,
) {
...
export function price2latestTick(
  price: number,
  token0decimals = 18,
  token1decimals = 18,
) {
  ...
  const closest = priceToClosestTick(
    new Price(
      ...
      JSBI.multiply(
        JSBI.BigInt(1),
        JSBI.exponentiate(JSBI.BigInt(10), JSBI.BigInt(token1decimals)),
      ),
    ),
  )
  return closest
...
export function price2latestUsableTick(
  price: number,
  token0decimals = 18,
  token1decimals = 18,
  tickSpacing: number,
